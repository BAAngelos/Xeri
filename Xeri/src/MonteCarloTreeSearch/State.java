...
public class State {
	...
	public void cleverPlay() {

		Card cardOnBoard = null;
		if (this.simulatedBoard.getField().size() != 0) {
			cardOnBoard = this.simulatedBoard.getField().get(this.simulatedBoard.getField().size() - 1);
		}
		...
		if (playerNo == 1) {
			if (cardOnBoard != null) {
				CardPile p1Hand = this.simulatedBoard.getP1Hand();
				for (Card card : p1Hand) {
					if (cardOnBoard.getValue() == card.getValue()) {
						cardsThatTakeTrick.add(card);
					}
				}
				if (cardsThatTakeTrick.size() > 0) {
					randomCard = cardsThatTakeTrick.get((int) (Math.random() * cardsThatTakeTrick.size()));
					this.simulatedBoard.performMove(this.playerNo, randomCard);
					return;
				}
			}
			...
			int cardsInHand = this.simulatedBoard.getP1Hand().size();
			randomCard = this.simulatedBoard.getP1Hand().get((int) (Math.random() * cardsInHand));
			this.simulatedBoard.performMove(this.playerNo, randomCard);

		} else if (playerNo == 2) {
			if (cardOnBoard != null) {
				CardPile p1Hand = this.simulatedBoard.getP2Hand();
				for (Card card : p1Hand) {
					if (cardOnBoard.getValue() == card.getValue()) {
						cardsThatTakeTrick.add(card);
					}
			...
			int possibleCard = this.simulatedBoard.getP2Hand().size();
			randomCard = simulatedBoard.getP2Hand().get((int) (Math.random() * possibleCard));
			this.simulatedBoard.performMove(this.playerNo, randomCard);
		}
	}
